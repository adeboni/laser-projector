"""This module generates data for the lasers"""
from threading import Thread
from queue import Queue
from typing import Callable
from flask import Flask, Response, current_app
import numpy as np
from utils.helper_functions import *
from utils.laser_point import *
from laser_generators import *

class LaserServer:
    """This class generates data for the lasers"""
    def __init__(self, num_lasers: int, callback: Callable):
        self.mode = 0
        self.num_lasers = num_lasers
        self.mode_list = {0: circle(), 1: rainbow_circle()}

        self.flask_app = Flask(__name__)
        self.flask_app.queues = [Queue(1024) for _ in range(self.num_lasers)]
        self.flask_app.callback = callback
        self.server = Thread(target=self.flask_app.run, kwargs={'host': '127.0.0.1', 'port': 8100}, daemon=True)
        self.gen = Thread(target=self.producer, kwargs={'queues': self.flask_app.queues}, daemon=True)
        
        @self.flask_app.route('/laser_data/<int:laser_id>/<int:num_points>/', methods = ['GET'])
        def get_laser_data(laser_id: int, num_points: int) -> Response:
            """Returns the next set of laser data"""
            return_buf = []
            new_points = []
            while len(return_buf) < num_points * 6:
                try:
                    point = current_app.queues[laser_id].get_nowait()
                    return_buf.extend(point.get_bytes())
                    new_points.append(point)
                except:
                    break
            if new_points and current_app.callback is not None:
                current_app.callback(laser_id, new_points)
            return Response(bytes(return_buf), mimetype='application/octet-stream')
        
    def start(self) -> None:
        """Starts the server and generator"""
        if not self.server.is_alive():
            self.server.start()

        if not self.gen.is_alive():
            self.gen.start()

    def producer(self, **kwargs) -> None:
        while True:
            if self.mode in self.mode_list and not kwargs['queues'][0].full():
                kwargs['queues'][0].put(next(self.mode_list[self.mode]))
